...
The bathymetry dataset in the GTSMv3.0 model is based on GEBCO_2014 (Muis et al., 2020), which is referenced to MSL. While there are newer (and possibly more accurate) global bathymetry datasets from GEBCO, we choose to use this dataset to stay consistent with the previous GTSM-ERA5 runs and to keep the same bathymetry source that the GTSMv3.0 model was calibrated with.
